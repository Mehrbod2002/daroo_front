...
var urlpath = searchparams.get("centerid");

if (urlpath) {
  var url = urldemo + `/api/admin/centers/details/${urlpath}/`;
  try {
    ...
    request.onloadend = function () {
      if (request.status == 200 || request.status == 201) {
        ...
        document.querySelector("#pepresentative_position").innerHTML =
          response.pepresentative_position;
        document.querySelector("#postal_code").innerHTML = response.postal_code;
        document.querySelector("#name").innerHTML = response.name;
        document.querySelector("#center_type").innerHTML = response.center_type;
        document.querySelector("#mobile").innerHTML = response.mobile;
        document.querySelector("#center_name").innerHTML = response.center_name;
        document.querySelector("#username").innerHTML = response.username;
        document.querySelector("#center_code").innerHTML = response.center_code;
        ...
        document.querySelector("#economy_code").innerHTML =
        ...
        if (response.wallet_blocked == "TRUE") {
          ...
          document.querySelector(
            "#walletStatus"
          ).innerHTML = ` <span> ${response.wallet_balance} ریال </span><button class="btn btn-secondary edit-wallet mx-1">
        ...
        document
          ...
          .setAttribute(
            "href",
            `/admin-requests-center.html?centerid=${response.id}`
          );
        document
          .querySelector("#profileEdit")
          .setAttribute("href", `/main-profile.html?centerid=${response.id}`);
        document
          .querySelector("#profileReport")
          .setAttribute("href", `/main-reports.html?centerid=${response.id}`);
        document
          .querySelector("#profileWallet")
          .setAttribute("href", `/main-wallet.html?centerid=${response.id}`);
        document
          .querySelector("#profileReception")
          .setAttribute(
            "href",
            `/main-reception-requests.html?centerid=${response.id}`
          );
